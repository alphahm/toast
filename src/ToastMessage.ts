...
// TODO: can we insert the content of a file instead?
template.innerHTML =  `
    <style>
        .toaster {
...
export default class ToastMessage extends HTMLElement {
    ...
    connectedCallback() {
        ...
        const divContentMsg = this.shadowRoot!.querySelector<HTMLDivElement>('.toast-content-message')
        divContentMsg!.textContent = this.textContent;
    }

    attributeChangedCallback(name: string, oldValue: string, newValue: string) {
        if (name == "show" && newValue == "no") {
            const wrapperDiv = this.shadowRoot!.querySelector<HTMLDivElement>('.toaster')
